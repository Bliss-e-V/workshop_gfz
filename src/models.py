...
class TemporalPredictionModel(nn.Module):
    ...
    def __init__(
        ...
        # Prediction head
        self.prediction_head = nn.Linear(hidden_channels, input_channels * (self.patch_size ** 2))
    ...
    def patchify(self, x):
        ...
        patches = self.patch_embed(x)  # (batch_size * time, hidden_channels, num_patches_h, num_patches_w)
        
        # Flatten spatial dimensions
        patches = patches.flatten(2).transpose(1, 2)  # (batch_size * time, num_patches, hidden_channels)
    ...
    def unpatchify(self, patches):
        ...
        patches = self.prediction_head(patches)  # (batch_size * time, num_patches, channels * patch_size^2)
        ...
        # Reshape back to include time dimension
        x = x.view(batch_size, time, channels, self.spatial_size[0], self.spatial_size[1])
    ...
    def forward(self, x):
        ...
        # Convert to patches
        patches = self.patchify(x)  # (batch_size, sequence_length, num_patches, hidden_channels)
        
        # Add positional encoding
        patches = patches + self.pos_embed.unsqueeze(1)  # Add time dimension to pos_embed
        ...
        for t in range(self.prediction_horizon):
            ...
            context_flat = context.view(batch_size, sequence_length * self.num_patches, self.hidden_channels)
            ...
            pred_patches = context_flat[:, -self.num_patches:, :]  # (batch_size, num_patches, hidden_channels)
            
            # Convert back to spatial format
            pred_spatial = self.unpatchify(pred_patches.unsqueeze(1))  # Add time dimension
            pred_spatial = pred_spatial.squeeze(1)  # Remove time dimension
            
            predictions.append(pred_spatial)
        
        # Stack predictions
        predictions = torch.stack(predictions, dim=1)  # (batch_size, prediction_horizon, channels, height, width)
...
class MaskedModelingModel(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_levels - 1):
            ...
            self.decoder.append(
                self._make_conv_block(out_channels * 2, out_channels)  # *2 for skip connection
            )
    ...
    def forward(self, x):
        ...
        # Decoder
        for i, (upsample, decoder_block) in enumerate(zip(self.decoder_upsamples, self.decoder)):
            x = upsample(x)

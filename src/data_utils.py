...
class EOBSDataLoader:
    ...
    def load_precipitation_data(self):
        ...
        precipitation_files = {
            ...
            'precipitation_spread': self.data_dir / "rr_ens_spread_0.25deg_reg_v31.0e.nc"
        }
...
class EOBSTemporalPredictionDataset(Dataset):
    ...
    def __init__(
        ...
        self.valid_start_indices = list(range(
            self.num_times - self.sequence_length - self.prediction_horizon + 1
        ))
    ...
    def _calculate_stats(self):
        ...
        sample_indices = random.sample(
            ...
            min(1000, len(self.valid_start_indices))
        )
        ...
        for idx in sample_indices:
            ...
            crop_data = self._get_random_spatial_crop(
                self.precip_var.isel(time=slice(idx, idx + self.sequence_length + self.prediction_horizon))
            )
    ...
    def __getitem__(self, idx):
        ...
        # Get time slice
        time_slice = slice(start_idx, start_idx + self.sequence_length + self.prediction_horizon)
        ...
        target = data_array[self.sequence_length:self.sequence_length + self.prediction_horizon]
...
class EOBSMaskedModelingDataset(Dataset):
    ...
    def _calculate_stats(self):
        ...
        sample_indices = random.sample(
            ...
            min(1000, len(self.valid_time_indices))
        )
